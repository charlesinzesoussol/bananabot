    ...
    def _add_command_handlers(self) -> None:
        ...
        async def _handle_help(ctx: commands.Context):
            ...
            embed = discord.Embed(
                ...
                description="AI Image Generation Bot with Gallery & Batch Processing",
                color=0xFFD700
            )
            
            embed.add_field(
                name="🎨 Generation Commands",
                value="`!generate <prompt>` - Generate image from text\\n`!gallery [limit]` - View your recent works",
                inline=False
            )
            
            embed.add_field(
                name="💡 Tips",
                value="• Attach images to `!generate` for editing\\n• All images saved to your personal gallery\\n• Use short, descriptive prompts for best results",
                inline=False
            )
            
            embed.set_footer(text="Made with 🍌 | Powered by Google Gemini 2.5 Flash Image")
